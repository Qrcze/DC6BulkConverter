using SixLabors.ImageSharp.PixelFormats;

namespace DC6BulkConverter
{
    public readonly struct DC6FrameHeader
    {
        /// <summary>
        /// If 0 - drawing goes from the bottom, if 1 - from the top. (both in lines/rows, going from left to right)
        /// </summary>
        public int Flip { get; }

        public int FrameWidth { get; }
        public int FrameHeight { get; }
        public int OffsetX { get; }
        public int OffsetY { get; }
        public int Unknown { get; }
        public int NextBlock { get; }
        public int Length { get; }

        /// <summary>
        /// "Encoded" pixel data.
        /// if byte == 0x80 -> go to next line
        /// if byte's first bit is 1 (so has a mask of 0x80) - the other 7 bits say how many pixels of transparency are to draw
        /// if byte's first bit is 0 - the 7 bits is a number of following bytes that will be colored
        /// </summary>
        public byte[] Data { get; }

        public DC6FrameHeader(ByteReader reader)
        {
            Flip = reader.ReadInt32();
            FrameWidth = reader.ReadInt32();
            FrameHeight = reader.ReadInt32();
            OffsetX = reader.ReadInt32();
            OffsetY = reader.ReadInt32();
            Unknown = reader.ReadInt32();
            NextBlock = reader.ReadInt32();
            Length = reader.ReadInt32();
            Data = reader.ReadBytes(Length);
        }

        internal Rgba32[] ToRgba32(int width, int height)
        {
            int len = width * height;

            var colorPixels = new Rgba32[len];

            int remainingPixels = width;
            int pixelsPosition = 0;
            for (int i = 0; i < Length; i++)
            {
                byte dataByte = Data[i];
                if (dataByte == 0x80)
                {
                    pixelsPosition += remainingPixels;
                    remainingPixels = width;
                    continue;
                }

                int pixels;
                if ((dataByte & 0x80) != 0) // transparency pixels
                {
                    pixels = (byte)(dataByte & ~0x80); //skip by however many pixels are transparent
                    pixelsPosition += pixels;
                }
                else //color pixels
                {
                    pixels = dataByte;
                    for (int j = 0; j < pixels; j++)
                    {
                        var colorIndex = Data[i + j + 1] * 3;
                        byte r = DefaultPalette[colorIndex + 2];
                        byte g = DefaultPalette[colorIndex + 1];
                        byte b = DefaultPalette[colorIndex];
                        colorPixels[pixelsPosition] = new(r, g, b);
                        pixelsPosition++;
                    }
                    i += pixels; //skip to the next non color pixels
                }

                remainingPixels -= pixels;
            }

            return colorPixels;
        }

        //BGR format; 768 elements (3*255 (+ 3 for last color GR values))
        public static readonly byte[] DefaultPalette =
        {
            
        };
    }
}